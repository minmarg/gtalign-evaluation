...
(ls -1 fatcat | grep -E .out | xargs -i -P 40 perl -e '
    ...
    if(eof(F) || /^Align\s+(\S+)\s+(\d+)\s+with\s+(\S+)\s+(\d+)/) {
      ...
      if($qid && $rfn && $qaln && $raln) {
        ...
        if(length($qaln) > 2 && length($raln) > 2 && $rlenloc > 2) {
          ...
          $prog = "$TMscore $QRYDIR/$qid $rfile -I $wfile >$tmfile";
          if(system($prog) != 0) {
  ...
  close(F);close(Q);') >fatcat__pdb20__processed_gdtts.out 2>fatcat__pdb20__processed_gdtts.err
...
(ls -1 deepalign | grep -E .out | xargs -i -P 40 perl -e '
  ...
  close(F);') >deepalign__pdb20__processed_gdtts.out 2>deepalign__pdb20__processed_gdtts.err
...
(ls -1 dali | grep -E .txt | xargs -i -P 40 perl -e '
  ...
  close(F);') >DALIv5__pdb20__processed_gdtts.out 2>DALIv5__pdb20__processed_gdtts.err
...
(ls -1 ${MYPROCDIR} | grep -E .out | xargs -i -P 40 perl -e '
  ...
  close(F);') >${MYPROCDIR}__pdb20__processed_gdtts.out 2>${MYPROCDIR}__pdb20__processed_gdtts.err
...
') >${MYPROCDIR}__pdb20__processed_gdtts.out 2>${MYPROCDIR}__pdb20__processed_gdtts.err
