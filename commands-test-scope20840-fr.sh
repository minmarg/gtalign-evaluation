...
function GetRecall1() {
  ...
  local ntotqrs=2045
  FQRY=$fqry FRFN=$frfn FMSR1=$fmsr1 FMSR2=$fmsr2 SRT=$srt HMN=$hmn LSTFILE=$lstfile NTOTQRS=$ntotqrs CFR=${CFR[@]} perl -e '
    @CFRs = ();
    ...
    foreach $q(keys %Queries) {
      ...
      $line = sprintf("%-10s %12s %6.4f %6.4f %6.4f  %5d  %5d %5d %5d", $q,$fam, $rec1fam,$rec1sfam,$rec1fold, $difffold, $szfam,$szsfam,$szfold);
      $Res{$line} = $rec1fold;
    }
    printf("%s %9.4f\n",$_,(++$c)/$ENV{NTOTQRS}) foreach (sort{$Res{$b}<=>$Res{$a}} keys %Res);
  ' $ifile >$ofile
}
